...
public interface HealthUnitRepository extends PagingAndSortingRepository<HealthUnit, Integer>  {
	...
	static final String HAVERSINE_PART = "(6371 * acos(cos(radians(:latitude)) * cos(radians(m.geocode.lat)) * cos(radians(m.geocode.lng) - "
	...
	@Query("SELECT m FROM HealthUnit m WHERE "+HAVERSINE_PART+" < 10000 ORDER BY "+HAVERSINE_PART+" DESC")
	 Page<HealthUnit> search(@Param("latitude") Float lat, @Param("longitude") Float lng, Pageable pageable);
